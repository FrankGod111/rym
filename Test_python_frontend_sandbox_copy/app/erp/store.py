...
class ERPStore:
    ...
    def register_dify_document(
        ...
        *,
        ...
        state = self.state()
        dify_dataset_id = str(state.get("settings", {}).get("dify_dataset_id") or "")
        ...
        payload = {
            ...
            "knowledge_dataset_key": "dify-remote",
    ...
    def record_dify_sync(
        self, document_id: str, result: dict[str, Any], dataset_id: str
    ) -> dict[str, Any]:
        with self._lock:
            self.ensure_layout()
            state = self._read_state_unsafe()
    ...
    def document_ai_gate(self, doc: dict[str, Any]) -> dict[str, Any]:
        ...
        if str(doc.get("confidentiality_level") or "") in {"sensitive", "restricted"}:
            return {"allowed": False, "block_reason": "敏感或受限文档默认禁止进入 AI"}
        required_metadata = {
        ...
        if str(doc.get("parse_status") or "") != "parsed":
    ...
    def _resolve_dataset_mapping(self, doc: dict[str, Any], state: dict[str, Any]) -> dict[str, Any] | None:
        ...
        for item in mappings:
            ...
            if document_type and document_type != doc_document_type:
                continue
            if ai_scope and ai_scope != doc_ai_scope:
                continue
            return item
        return None
