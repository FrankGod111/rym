...
ERP Knowledge Console MVP 是一个基于 FastAPI 的企业文档管理与知识库控制台。系统负责文档上传、文档管理、权限入口、审计日志和业务元数据；Dify 负责知识库切片、Embedding、索引、检索和 AI 问答。

当前版本适合本地 MVP 验证，不依赖 OpenClaw。当前已经补上基础 Bearer Token 鉴权、角色权限校验和 AI 服务端身份收口；生产化时仍建议继续替换为 PostgreSQL、对象存储、正式认证体系和更强的 Dify 隔离策略。
...
- Storage: 本地 JSON + 本地文件目录
- Dify API Client: httpx
- Knowledge Engine: Dify Knowledge Base
...
演示登录账号为 `admin`，密码使用任意非空值，例如 `admin`。
